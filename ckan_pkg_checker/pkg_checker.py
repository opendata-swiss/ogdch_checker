...
class PackageCheck:
    def __init__(
        self, config, siteurl, apikey, rundir, mode, limit, pkg, org, harvester_type
    ):
    ...
    def _get_packages(self, limit=None, pkg=None, org=None):
        ...
        if org and self.harvester_type:
            return self._get_org_and_harvester_package_ids(
                org, self.harvester_type, limit
            )
        if org:
        ...
        if self.harvester_type == "geocat":
            return (
                self._get_geocat_package_ids()[:limit]
                if limit
                else self._get_geocat_package_ids()
            )
    ...
    def _get_org_and_harvester_package_ids(self, org, harvester_type, limit=None):
        """
